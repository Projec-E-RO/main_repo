...
void InitMotorParameter(void)               // If using MDUI
{
    switch(byCntInitStep)
    {
        case SETTING_PARAM_STEP_PID_PNT_VEL_CMD:
        {
            ...
            // 두 번째 드라이버에도 동일 초기화 명령 전송
            PutMdData(PID_PNT_VEL_CMD,robotParamData.nRMID2,(const uint8_t *)&cmd_data, sizeof(cmd_data));
    ...
    switch(byCntComStep)
    {
        case 0:
        {
            ...
            if(velCmdUpdateCount > 0) 
            {
                ...
                PutMdData(PID_PNT_VEL_CMD, robotParamData.nRMID2, (const uint8_t *)&pid_pnt_vel_cmd, sizeof(pid_pnt_vel_cmd));
        ...
        case 3:
        {
            if(robotParamData.use_MDUI == 1) {              // Use only if you use MDUI
                if(curr_pid_robot_monitor.byPlatStatus.bits.bEmerSW == 1)
                {
                    ...
                    PutMdData(PID_PNT_TQ_OFF, robotParamData.nRMID2, (const uint8_t *)&pid_pnt_tq_off, sizeof(pid_pnt_tq_off));
                }
            }
            ...
            if(reset_pos_flag == true) {
                ...
                dummy = 0;
                PutMdData(PID_POSI_RESET, robotParamData.nRMID, (const uint8_t *)&dummy, sizeof(dummy));
                PutMdData(PID_POSI_RESET, robotParamData.nRMID2, (const uint8_t *)&dummy, sizeof(dummy));
                ...
                PutMdData(PID_COMMAND, robotParamData.nRMID2, (const uint8_t *)&cmd_pid, 1);
            }
        ...
        case 4:
        {
            ...
            if(robotParamData.use_MDUI == 0) { 
                ...
                PutMdData(PID_REQ_PID_DATA, robotParamData.nRMID2, (const uint8_t *)&request_pid, 1);
            }
            else {
...
int main(int argc, char** argv)
{
    ...
    nh.getParam("md_robot_node/second_driver_id", robotParamData.nIDMDT2);
    robotParamData.nRMID2 = robotParamData.nIDMDT2;
    ROS_INFO("Second driver ID       : %d", robotParamData.nIDMDT2);
