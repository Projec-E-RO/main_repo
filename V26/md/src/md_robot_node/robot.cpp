...
int16_t * RobotSpeedToRPMSpeed(double linear, double angular)
{
    ...
    static int16_t goal_rpm_speed[4]; //추가
    ...
    wheel_velocity_cmd[LEFT]   = linear - (angular * wheel_separation / 2); //좌: 모터드라이버
    wheel_velocity_cmd[RIGHT]  = linear + (angular * wheel_separation / 2); // 우: 모터드라이버
    ...
    goal_rpm_speed[0] = (int16_t)(wheel_velocity_cmd[LEFT]); //모터1
    goal_rpm_speed[1] = (int16_t)(wheel_velocity_cmd[RIGHT]); //모터2
    goal_rpm_speed[2] = (int16_t)(wheel_velocity_cmd[LEFT]); //모터3
    goal_rpm_speed[3] = (int16_t)(wheel_velocity_cmd[RIGHT]); //모터4
    //추가
    ROS_INFO("RPMs: L1=%d, L2=%d, R1=%d, R2=%d",
        goal_rpm_speed[0],
        goal_rpm_speed[1],
        goal_rpm_speed[2],
        goal_rpm_speed[3]);
