...
void InitMotorParameter(void)               // If using MDUI
{
    switch (byCntInitStep)
    {
        ...
        {
            PID_PNT_VEL_CMD_t cmd_data, *p = &cmd_data;
            ...
            p->enable_id2     = 1;
            p->rpm_id2        = 0;
            cmd_data.req_monitor_id = (robotParamData.use_MDUI == 1
                                       ? REQUEST_PID_ROBOT_MONITOR
                                       : REQUEST_PNT_MAIN_DATA);

            pid_response_receive_count = 0;
            pid_request_cmd_vel_count  = 1;

            // 첫 번째 모터드라이버(ID=1) 전송
            PutMdDataPerID(PID_PNT_VEL_CMD,
                           1,
                           (const uint8_t *)p,
                           sizeof(cmd_data));
            // 두 번째 모터드라이버(ID=20) 전송
            PutMdDataPerID(PID_PNT_VEL_CMD,
                           20,
                           (const uint8_t *)p,
                           sizeof(cmd_data));
        ...
        case SETTING_PARAM_STEP_PID_ROBOT_PARAM:
        {
            if (robotParamData.use_MDUI == 1) {
                ...
                ROS_INFO("[SET] PID_ROBOT_PARAM(PID NO: %d)", PID_ROBOT_PARAM);
                cmd_data.nDiameter    = (uint16_t)robotParamData.nDiameter;
                cmd_data.nWheelLength = (uint16_t)(robotParamData.nWheelLength * 1000.0);
                cmd_data.nGearRatio   = (uint16_t)robotParamData.nGearRatio;

                PutMdData(PID_ROBOT_PARAM,
                          MID_MDUI,
        ...
        case SETTING_PARAM_STEP_PID_POSI_RESET:
        {
            ...
            PutMdData(PID_POSI_RESET,
                      robotParamData.nRMID,
                      &dummy,
                      sizeof(dummy));
        ...
        {
            PID_SLOW_START_t cmd_data{ .value = (uint16_t)robotParamData.nSlowstart };
            ROS_INFO("[SET] PID_SLOW_START(PID NO: %d)", PID_SLOW_START);
            PutMdData(PID_SLOW_START,
                      robotParamData.nRMID,
                      (const uint8_t *)&cmd_data,
                      sizeof(cmd_data));
        ...
        {
            PID_SLOW_DOWN_t cmd_data{ .value = (uint16_t)robotParamData.nSlowdown };
            ROS_INFO("[SET] PID_SLOW_DOWN(PID NO: %d)", PID_SLOW_DOWN);
            PutMdData(PID_SLOW_DOWN,
                      robotParamData.nRMID,
                      (const uint8_t *)&cmd_data,
                      sizeof(cmd_data));
        ...
        {
            uint8_t cmd_data = robotParamData.reverse_direction ? 1 : 0;
            ROS_INFO("[SET] PID_INV_SIGN_CMD(PID NO: %d)", PID_INV_SIGN_CMD);
            PutMdData(PID_INV_SIGN_CMD,
                      robotParamData.nRMID,
                      &cmd_data,
                      1);
        ...
        {
            uint8_t cmd_data = robotParamData.enable_encoder ? 1 : 0;
            ROS_INFO("[SET] PID_USE_POSI(PID NO: %d)", PID_USE_POSI);
            PutMdData(PID_USE_POSI,
                      robotParamData.nRMID,
                      &cmd_data,
                      1);
        ...
        case SETTING_PARAM_STEP_PID_PPR:
        {
            if (robotParamData.use_MDUI == 1 && robotParamData.enable_encoder == 1) {
                PID_PPR_t cmd_data{ .PPR = robotParamData.encoder_PPR };
            ...
            byCntInitStep = SETTING_PARAM_STEP_DONE;
            fgInitsetting = (pid_request_cmd_vel_count == 2
                             ? INIT_SETTING_STATE_OK
                             : INIT_SETTING_STATE_ERROR);
            break;
        }
    ...
    switch(byCntComStep)
    {
        case 0:
        {
            ...
            if(velCmdUpdateCount > 0) 
            {
                ...
                PutMdDataPerID(PID_PNT_VEL_CMD, 1, (const uint8_t *)&pid_pnt_vel_cmd, sizeof(pid_pnt_vel_cmd));
                // ─── 두 번째 드라이버 (모터3,4) ───
                p->enable_id1 = 1;
                p->rpm_id1 = pGoalRPMSpeed[2];
                p->enable_id2 = 1;
                p->rpm_id2 = pGoalRPMSpeed[3];
                PutMdDataPerID(PID_PNT_VEL_CMD, 20, (const uint8_t *)&pid_pnt_vel_cmd, sizeof(pid_pnt_vel_cmd));
        ...
        case 3:
        {
            if(robotParamData.use_MDUI == 1) {              // Use only if you use MDUI
                if(curr_pid_robot_monitor.byPlatStatus.bits.bEmerSW == 1)
                {
                    ...
                    PutMdDataPerID(PID_PNT_TQ_OFF, 1, (const uint8_t *)&pid_pnt_tq_off, sizeof(pid_pnt_tq_off));
                    PutMdDataPerID(PID_PNT_TQ_OFF, 20, (const uint8_t *)&pid_pnt_tq_off, sizeof(pid_pnt_tq_off));
                }
            }
            ...
            if(reset_pos_flag == true) {
                ...
                dummy = 0;
                PutMdDataPerID(PID_POSI_RESET, 1, (const uint8_t *)&dummy, sizeof(dummy));
                PutMdDataPerID(PID_POSI_RESET, 20, (const uint8_t *)&dummy, sizeof(dummy));
                ...
                PutMdDataPerID(PID_COMMAND, 1, (const uint8_t *)&cmd_pid, 1);
                PutMdDataPerID(PID_COMMAND, 20, (const uint8_t *)&cmd_pid, 1);
            }
        ...
        case 4:
        {
            ...
            if(robotParamData.use_MDUI == 0) { 
                ...
                PutMdDataPerID(PID_REQ_PID_DATA, 1, (const uint8_t *)&request_pid, 1);
                PutMdDataPerID(PID_REQ_PID_DATA, 20, (const uint8_t *)&request_pid, 1);
            }
            else {
...
int main(int argc, char** argv)
{
    ...
    robotParamData.nIDMDT  = MID_MDT;        // MDT ID
